...
public class XmlDataProvider : IDataProvider
{
    ...
    protected XmlSource<ExpandoObject> _dataFlowExecutableSource = null!;

    public string Name => DataProvider.Xml;
    ...
    protected virtual void SetConnectionString(string connectionString)
    {
        ...
        {
            throw new Exception("'Path' is a required setting for Xml data provider in 'Options'");
        }
        ...
        if (options.ContainsKey("source"))
        {
            _resourceType = options["source"].ToLower() switch
            {
                ...
                _ => throw new Exception($"Invalid Xml source '{options["source"]}' specified in 'Options'")
            };
        }
        else
        {
