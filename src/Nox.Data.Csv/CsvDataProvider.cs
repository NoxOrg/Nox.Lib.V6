...
public class CsvDataProvider : IDataProvider
{
    ...
    public string Name => DataProvider.Csv;
    ...
    protected virtual void SetConnectionString(string connectionString)
    {
        ...
        {
            throw new Exception("'Path' is a required setting for Csv data provider in 'Options'");
        }
        ...
        if (options.ContainsKey("source"))
        {
            _resourceType = options["source"].ToLower() switch
            {
                ...
                _ => throw new Exception($"Invalid Csv source '{options["source"]}' specified in 'Options'")
            };
        }
        else
        {
