...
public class NoxDynamicGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        else
        {
            ...
            foreach (Dictionary<object, object>? entity in entities)
            {
                ...
                {
                    AddDomainEvent(context, assemblyName!, GeneratorEventType.Created, entity!);
                    AddDomainEvent(context, assemblyName!, GeneratorEventType.Updated, entity!);
                    AddDomainEvent(context, assemblyName!, GeneratorEventType.Deleted, entity!);    
                }
            }    
        }
    ...
    private void AddDomainEvent(GeneratorExecutionContext context, string assemblyName, GeneratorEventType generatorEventType, Dictionary<object, object>? entity)
    {
        var sb = new StringBuilder();

        var eventTypeName = generatorEventType.ToString();

        var className = $"{entity!["name"]}{eventTypeName}DomainEvent";

        sb.AppendLine($@"// autogenerated");
        sb.AppendLine($@"using Nox.Core.Interfaces.Messaging.Events;");
        sb.AppendLine($@"");
        sb.AppendLine($@"namespace Nox;");
        sb.AppendLine($@"");
