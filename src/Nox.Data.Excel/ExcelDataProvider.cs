...
public class ExcelDataProvider : IDataProvider
{
    ...
    protected ExcelSource<ExpandoObject> _dataFlowExecutableSource = null!;

    public string Name => DataProvider.Excel;
    ...
    protected virtual void SetConnectionString(string connectionString)
    {
        ...
        {
            throw new Exception("'Path' is a required setting for Excel (.xls/.xlsx) data provider in 'Options'");
        }
        ...
        if (options.ContainsKey("source"))
        {
            _resourceType = options["source"].ToLower() switch
            {
                ...
                _ => throw new Exception($"Invalid Excel source '{options["source"]}' specified in 'Options'")
            };
        }
        else
        {
